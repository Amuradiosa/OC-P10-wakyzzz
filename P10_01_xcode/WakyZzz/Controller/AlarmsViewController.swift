...
//        print(FileManager.default.urls(for: .documentDirectory, in: .userDomainMask))
    ...
    func alarmCell(_ cell: AlarmTableViewCell, enabledChanged enabled: Bool) {
        if let indexPath = tableView.indexPath(for: cell) {
            if let alarm = alarm(at: indexPath) {
                ...
                alarm.enabled ? appDelegate?.manageSettingUpLocalNotificationFor(alarm) : appDelegate?.removePendingNotificationFor(alarmID: alarm.creationDateID!)
    ...
    func addOrEdit(_ alarm: Alarm, at indexPath: IndexPath? = nil) {
            ...
            if indexPath != nil {
                ...
                newAlarm.creationDateID = alarm.creationDateID!
    ...
    func loadAlarmsFromCoreDataAndPopulateTableView() {
        ...
        for i in savedAlarms {
            ...
            alarm.creationDateID = i.creationDateID
            alarms.append(alarm)
        }
        ...
        if CoreDataManager.shared.fetchedRC.fetchedObjects?.count == 2 {
            for i in alarms {
